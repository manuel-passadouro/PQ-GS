...
{
if(a & 1)
    GPIOPinWrite(GPIO_PORTB_BASE, D4, D4);
else
    GPIOPinWrite(GPIO_PORTB_BASE, D4, 0);
if(a & 2)
    GPIOPinWrite(GPIO_PORTB_BASE, D5, D5);
else
    GPIOPinWrite(GPIO_PORTB_BASE, D5, 0);
if(a & 4)
    GPIOPinWrite(GPIO_PORTB_BASE, D6, D6);
else
    GPIOPinWrite(GPIO_PORTB_BASE, D6, 0);
if(a & 8)
    GPIOPinWrite(GPIO_PORTB_BASE, D7, D7);
else
    GPIOPinWrite(GPIO_PORTB_BASE, D7, 0);
}
/**************************************************************
...
Lcd_Port(a);
GPIOPinWrite(GPIO_PORTC_BASE, EN, EN); // => E = 1
SysCtlDelay(26667);
GPIOPinWrite(GPIO_PORTC_BASE, EN, 0); // => E = 0
...
temp = a&0x0F;
y = a&0xF0;
GPIOPinWrite(GPIO_PORTE_BASE, RS, RS ); // => RS = 1
Lcd_Port(y>>4); //Data transfer
GPIOPinWrite(GPIO_PORTC_BASE, EN, EN); //EN =1
SysCtlDelay(10000);
GPIOPinWrite(GPIO_PORTC_BASE, EN, 0);//EN = 0;
Lcd_Port(temp);
GPIOPinWrite(GPIO_PORTC_BASE, EN, EN);//EN = 1;
SysCtlDelay(400);
GPIOPinWrite(GPIO_PORTC_BASE, EN, 0);//EN = 0;
